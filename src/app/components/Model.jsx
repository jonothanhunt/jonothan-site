...
// Utility function for color transitions
const getColorForTimeOfDay = (timeInSeconds) => {
  const dayDuration = 60 * 60;
  const timeInDay = timeInSeconds % dayDuration;
  const progress = timeInDay / dayDuration;
  const colorStart = new THREE.Color(1.0, 1.0, 1.0);
  const colorEnd = new THREE.Color(0.2, 0.0, 0.5);
...
export function Model(props) {
  ...
  useFrame((state, delta) => {
    ...
    [portalRefBackground, portalRefForeground].forEach((ref) => {
      if (ref.current) {
