...
interface SplashCursorProps {
  ...
  BACK_COLOR?: ColorRGB;
  TRANSPARENT?: boolean;
  PIXELATE_FACTOR?: number; // New prop for controlling pixelation
}
...
export default function SplashCursor({
  SIM_RESOLUTION = 16, // Reduced from 16 to make simulation more blocky
  DYE_RESOLUTION = 16, // Reduced from 16 to make colors more blocky
  CAPTURE_RESOLUTION = 16,
  ...
  SPLAT_RADIUS = 0.6,
  SPLAT_FORCE = 600,
  SHADING = false, // Disabled shading to make blocks more uniform
  COLOR_UPDATE_SPEED = 10,
  BACK_COLOR = { r: 0, g: 0, b: 0 },
  TRANSPARENT = true,
  PIXELATE_FACTOR = 264, // Controls the size of the pixelated blocks
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (isWebGL2) {
        ...
        // Force nearest neighbor filtering for pixelated look
  ...
  `
    ...
    void main () {
        // Pixelate by rounding the texture coordinates to a grid
        vec2 pixelatedUv = floor(vUv * pixelateFactor) / pixelateFactor;
        
        vec3 c = texture2D(uTexture, pixelatedUv).rgb;
        
        #ifdef SHADING
            vec3 lc = texture2D(uTexture, floor(vL * pixelateFactor) / pixelateFactor).rgb;
            vec3 rc = texture2D(uTexture, floor(vR * pixelateFactor) / pixelateFactor).rgb;
            vec3 tc = texture2D(uTexture, floor(vT * pixelateFactor) / pixelateFactor).rgb;
            vec3 bc = texture2D(uTexture, floor(vB * pixelateFactor) / pixelateFactor).rgb;
  ...
  `
    ...
    function initFramebuffers() {
      ...
      // Always use NEAREST filtering for pixelated look
      ...
      if (!dye) {
        dye = createDoubleFBO(
          dyeRes.width,
          dyeRes.height,
          (rgba as { internalFormat: number }).internalFormat,
      ...
      if (!velocity) {
        velocity = createDoubleFBO(
          simRes.width,
          simRes.height,
          (rg as { internalFormat: number }).internalFormat,
      ...
      divergence = createFBO(
        simRes.width,
        simRes.height,
        (r as { internalFormat: number }).internalFormat,
    ...
    function drawDisplay(target: FBO | null) {
      ...
      // Add pixelation factor uniform
      if (displayMaterial.uniforms.pixelateFactor) {
        gl.uniform1f(
          displayMaterial.uniforms.pixelateFactor,
          config.PIXELATE_FACTOR
        );
      }
      blit(target, false);
    }
